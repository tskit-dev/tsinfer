...
def get_output_slice_path(path, input_path, start, end):
    return get_default_path(path, input_path, f".{start}-{end}.slice")
...
def run_match_sample_slice(args, usage_summary=True):
    setup_logging(args)

    sample_data = tsinfer.SampleData.load(args.samples)
    ancestors_trees = get_ancestors_trees_path(args.ancestors_trees, args.samples)
    output_slice = get_output_slice_path(
        args.output_slice_file, args.samples, args.start, args.end
    )
    logger.info(f"Loading ancestral genealogies from {ancestors_trees}")
    ancestors_trees = tskit.load(ancestors_trees)
...
def run_combine_sample_slices(args, usage_summary=True):
    setup_logging(args)
...
def add_output_slice_file_argument(parser):
    parser.add_argument(
        "--output-slice-file",
        default=None,
        help=(
            "The path to the output paths file."
            "If not specified, this defaults to the input samples file stem "
            "with the extension '{start}-{end}.slice'. For example, if "
            "'1kg-chr1.samples' is the input file then the default slice file would be "
            "'1kg-chr1.{start}-{end}.slice'"
        ),
    )
...
def add_slice_arguments(parser):
    parser.add_argument(
        "--start",
        type=int,
        help="The start index of the sample slice, inclusive",
    )
    parser.add_argument(
        "--end",
        type=int,
        help="The end index of the sample slice, exclusive",
    )


def add_slice_files_argument(parser):
    parser.add_argument(
        "slice_files",
        nargs="+",
        help=("One or more paths files generated by the match-samples-slice command."),
    )
...
def get_cli_parser():
    ...
    parser = subparsers.add_parser(
        "match-sample-slice",
        help=(
            "Matches a slice of samples against the tree sequence structure built "
            "by the match-ancestors command, saving the paths found to a file. Used"
            "to parallelise the sample matching process. Use combine-sample-slices to"
            "combine the results into a single tree sequence."
        ),
    )
    add_samples_file_argument(parser)
    add_output_slice_file_argument(parser)
    add_slice_arguments(parser)
    add_logging_arguments(parser)
    add_ancestors_trees_argument(parser)
    add_path_compression_argument(parser)
    add_num_threads_argument(parser)
    ...
    add_recombination_arguments(parser)
    add_mismatch_argument(parser)
    parser.set_defaults(runner=run_match_sample_slice)

    parser = subparsers.add_parser(
        "combine-sample-slices",
        help=(
            "Adds samples to the tree sequence built by match-ancestors, using the "
            "paths found by match-sample-slice. Used to parallelise the sample "
            "matching process."
        ),
    )
    add_samples_file_argument(parser)
    add_slice_files_argument(parser)
    add_logging_arguments(parser)
    add_ancestors_trees_argument(parser)
    add_path_compression_argument(parser)
    add_postprocess_argument(parser)
    ...
    add_recombination_arguments(parser)
    add_mismatch_argument(parser)
    parser.set_defaults(runner=run_combine_sample_slices)
