...
AUTOGENERATED_POP_NAME_PREFIX = "pop_"
AUTOGENERATED_POP_DESCRIPTION = "name autogenerated by tsinfer"
...
class SampleData(DataContainer):
    ...
    def add_population(self, metadata=None, name=None, description=None):
        ...
        :param str name: A unique name for this population (of length > 0). If given,
            ...
            population 0 will be given the name `pop_0`, population 1 `pop_1`, etc.
        :param str description: A description for this population (default: None). If not
            ...
            set to a default string (given by `tsinfer.AUTOGENERATED_POP_DESCRIPTION`),
            whereas if the name was  not autogenerated the description will default
            to the empty string.

        :return: The ID of the newly added population.
        :rtype: int
        :raises: ValueError if the name has already been used for another population.
        """
        ...
        if "name" not in metadata:
            metadata["name"] = AUTOGENERATED_POP_NAME_PREFIX + str(
                len(self._population_names)
            )
            if description not in metadata and description is None:
                metadata["description"] = AUTOGENERATED_POP_DESCRIPTION
        if metadata["name"] in self._population_names:
            raise ValueError(f"Another population is already named {metadata['name']}")
        self._population_names.add(metadata["name"])
    ...
    def merge(self, other, *, use_population_names=False, **kwargs):
        """
        Returns a copy of this SampleData instance merged with the specified
        other SampleData instance. Subsequent keyword arguments are passed
        to the SampleData constructor for the returned merged dataset.

        The datasets are merged by following process:

        1. We add the populations from this dataset to the result. If
           ``use_population_names`` is False, all populations in ``other`` will
           then be added as new, distinct populations (and named accordingly if
           necessary, see note below). Otherwise populations with the same name
           in the two datasets will be merged and only populations with novel
           names will be added to the result. In either case, references to the
           population IDs are adjusted as necessary.
        2. We add individual data from this dataset to the result, followed
           by the individuals from the other dataset, and adjust references to
           individual IDs as necessary.
        ...
        :param SampleData other: The other :class:`SampleData` instance to
            to merge.
        :param bool use_population_names: If ``False`` (default) always treat
            populations in ``other`` as distinct populations. If ``True``,
            populations in ``other`` that have the same metadata name are
            treated as the same population.
        :return: A new SampleData instance which contains the merged data
            from the two datasets.
        :rtype: .SampleData
        :raises: ValueError if ``other`` has a different sequence length.
        :raises: ValueError if during auto-generation of names, a population in
            `other` is allocated a new name which is the same as the name of a
            population in the newly created dataset (see note).

        .. note::
            In order to ensure names are unique, if `use_population_names` is False,
            and a population in `other` has the same name as one in the dataset being
            generated, a new name will be auto-generated for the population,
            in the same format as described in :meth:`add_population`.
        ...
        with SampleData(sequence_length=self.sequence_length, **kwargs) as result:
            ...
            for p in self.populations():
                result.add_population(p.metadata)
                num_populations += 1
                pop_name_map[p.metadata["name"]] = p.id
            pop_id_map = {-1: -1}
            for p in other.populations():
                if p.metadata["name"] in pop_name_map:
                    if use_population_names:
                        # Merge the populations; don't create a new one
                        pop_id_map[p.id] = pop_name_map[p.metadata["name"]]
                        continue
                    else:
                        # Name clash: keep the population but allocate a new name
                        p.metadata["name"] = AUTOGENERATED_POP_NAME_PREFIX + str(
                            num_populations
                        )
                pop_id_map[p.id] = result.add_population(p.metadata)
                pop_name_map[p.metadata["name"]] = None  # Add name to avoid later dups
                num_populations += 1

            result.__insert_individuals(self)
            result.__insert_individuals(other, pop_id_map)
