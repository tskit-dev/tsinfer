...
def insert_matches(tables, time, all_positions, matches):
    ...
    for m, new_sites in matches:
        u = tables.nodes.add_row(time=time, flags=1)
...
if __name__ == "__main__":
    ts = msprime.sim_ancestry(
        15, recombination_rate=0.001, sequence_length=10000, random_seed=32
    )
    ts_orig = msprime.sim_mutations(ts, rate=0.001, random_seed=41)
    # print(ts_orig)
    ...
    # FIXME not quite working - something wrong with how we're handling singletons
